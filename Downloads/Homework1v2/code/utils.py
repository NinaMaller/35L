...
def imresize(input_image, target_size):
    ...
    output_image = cv2.normalize(src = resized_image, alpha=-1, beta=1)
    print('Resized Dimensions : ',resized_image.shape)
 
    # debug: check that it works
...
def reportAccuracy(true_labels, predicted_labels, label_dict):
    ...
    # accuracy is # equal between both arrays / total number of rows
    accuracy = (true_labels == predicted_labels) / true_labels.shape[0]
    return accuracy
